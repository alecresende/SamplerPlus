...
Wav.prototype.setBuffer = function(buffer){
	var t = new Float32Array(buffer[0].length * 2);
	for(var i =0 ; i < buffer[0].length ; i++) {
		t[2*i] = buffer[0][i];
		t[2*i+1] = buffer[1][i];
	}
	this._buffer = this.getWavInt16Array(t); 
...
function makeWAV(buffer, sampleRate) {
	
	var chan1 = buffer.getChannelData(0);
	var chan2 = buffer.getChannelData(1);

	var i=0, vol=0.1,
		wav = new Wav({sampleRate: sampleRate, channels: 2});
	
	wav.setBuffer([chan1,chan2]);
	console.log("SWEET");
